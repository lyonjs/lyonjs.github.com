...
(function($, Raphael, location, exports) {
  ...
  $(function() {
    ...
    // draw the svg
    var paper = new Raphael($('#logo')[0], '100%', '100%');
    var anim = anims[Math.floor(Math.random() * anims.length)];
    var hover = /yay/.test(location.hash) ? hovers[hovers.length - 1] : hovers[Math.floor(Math.random() * hovers.length)];
    ...
    function rd() {return Math.floor(Math.random() * 600)}
    function animated() {this._animated = false}


    var desc = $('.eb-description'),
